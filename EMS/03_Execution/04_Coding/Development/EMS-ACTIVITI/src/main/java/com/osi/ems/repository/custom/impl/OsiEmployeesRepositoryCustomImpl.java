...
public class OsiEmployeesRepositoryCustomImpl implements OsiEmployeesRepositoryCustom {
		...
		public OsiEmployees getEmployee(int employeeId) throws DataAccessException {
			...
			try {
					String query = "select e.employee_id,e.employee_number,e.first_name,e.last_name, e.start_date, e.title, e.employee_type, e.office_email,"
							...
							+ " concat(COALESCE(c.country_code,''),c.contact_number) as contact_number, g.grade_name, e.org_id, e.full_name, e.user_name "
							...
							+ " left join osi_rollups d on d.rollup_id = a.dept_id " 
				...
				throw new DataAccessException("ERR_1002", "Exception occured in getting the list of employee with id: "+employeeId);
			} catch (Exception e) {
		...
		public Map<String, String> getModifiedFields(int employeeId) throws DataAccessException {
			...
			String modifiedFieldsQuery = "select"
					...
					+ " case when pdept_id != dept_id then (select concat(if(d.SEGMENT1 is null, '', concat(d.SEGMENT1, '.')), if(d.SEGMENT2 is null, '', concat(d.SEGMENT2, '.')), if(d.SEGMENT3 is null, '', concat(d.SEGMENT3, '.')), if(d.SEGMENT4 is null, '', concat(d.SEGMENT4, '.')), if(d.SEGMENT5 is null, '', concat(d.SEGMENT5, '.')), if(d.SEGMENT6 is null, '', concat(d.SEGMENT6, '.')),  if(d.SEGMENT7 is null, '', concat(d.SEGMENT7, '.')),if(d.SEGMENT8 is null, '', concat(d.SEGMENT8, '.')), if(d.SEGMENT9 is null, '', concat(d.SEGMENT9, '.')),if(d.SEGMENT10 is null, '', concat(d.SEGMENT10, '.'))  )from osi_rollups d where d.rollup_id = c.dept_id) end dept, "
					+ " (select concat(country_code, ' ', contact_number) mobile from osi_contacts"
					+ " where employee_id=?1 and contact_type='personal') mobile"
					+ " from ("
					...
					+ " where assignment.employee_id = ?2 and assignment.assignment_id not in (select assignment_id from osi_assignments where employee_id = assignment.employee_id and current_timestamp between effective_start_date and effective_end_date)  order by assignment_id desc limit 1"
					+ " ) a,"
					+ " (select supervisor_id, grade_id, job_id, location_id, dept_id from osi_assignments where employee_id = ?3 and current_timestamp between effective_start_date and effective_end_date) b"
					+ " )c";
			try {
		...
		public List<OsiEmployees> getEmployeesNotUpdatedSkills() throws DataAccessException {
			...
			try {
				...
				List<Object[]> outdatedSkills = this.entityManager.createNativeQuery(query)
													...
													.setParameter(2, dueDateInMonths * 30)
													.getResultList();
		...
		public List<OsiEmployees> getOnProbationEmployees() throws DataAccessException {
			...
			String onProbationEmpListQuery = "select e.employee_id as empId, e.employee_number, e.full_name name, e.office_email, e1.full_name supervisor, " + 
					...
					" a.location_id,l.location_name, a.pay_grade_id,g.grade_name, e.start_date, concat(if(c.country_code is null, '', c.country_code),' ', c.contact_number) mobile, e1.employee_id " + 
					...
					" left join osi_locations l on l.location_id = a.location_id " + 
					" left join osi_grades g on g.grade_id = a.pay_grade_id " + 
					" left join osi_rollups r on r.ROLLUP_ID = a.dept_id " + 
					" left join osi_contacts c on c.employee_id = e.employee_id and c.contact_type='personal' " + 
...
//					" end pdate" + 
					...
					+ " and assignment.on_probation = 1 and CURRENT_DATE() <= assignment.probation_end_date) d)";
			try {
