...
vm.validateForm = function(){
	...
	// }else
  if(vm.personalInfo.personalEmail === undefined ||vm.personalInfo.personalEmail == null || vm.personalInfo.personalEmail === ''){
		...
		vm.displayErrorMsg('Mobile Number Country code Field is required','error');
		$scope.verifyData = false;
  ...
  // else if(vm.personalInfo.maritalStatus === undefined || vm.personalInfo.maritalStatus == null || vm.personalInfo.maritalStatus ===''){
	...
	// 	$scope.verifyData = false;
	// }
	else if(vm.personalInfo.totalExp === undefined || vm.personalInfo.totalExp == null || vm.personalInfo.totalExp === '') {
...
/*	   vm.validateAddress(true);
  ...
  function getPersonalInfo(inputObject) {
	...
	function loadFile(fileUpload) {
			...
			$rootScope.isTrascErrorForPersonal = false;
			if (fileUpload) {
				if (fileUpload && (fileUpload.type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')) {
