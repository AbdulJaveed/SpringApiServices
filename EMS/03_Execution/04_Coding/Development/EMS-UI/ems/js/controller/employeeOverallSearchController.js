...
vm.addSearchField = function () {
	...
	if($scope.searchField) {
		...
		if(addFlag) {
			var t = "<div class='col-lg-5' id='"+$scope.searchField+idx+"Div'>"+
			"<button ng-click=vm.remove('"+$scope.searchField+idx+"')><span class='glyphicon glyphicon-remove'></span></button>"+
			"<input class='form-control' name='"+$scope.searchField+idx+"' id='"+$scope.searchField+idx+"' placeholder='"+capitalizeFirstLetter($scope.searchField+idx)+"' type='text'>"+
			"<br/></div>";
