(function () {
    ...
    function operationExclusionService($http,$cookieStore, $rootScope, $timeout, $localStorage, $sessionStorage,configData)
    {
        ...
        function getExcludedOperations(id){
            return $http.get(configData.url+'osi-user-operation-excl/'+id,config).then(handleSuccess, handleError('Error getting Excluded Operations'));
        }
