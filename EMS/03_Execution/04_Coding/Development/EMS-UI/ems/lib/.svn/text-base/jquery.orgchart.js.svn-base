...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    var $chart = $('<div>', {
      ...
      'click': function(event) {
        ...
        /*$chart.find('.org-add-button').click(function(){
            if(opts.onClickNode !== null){
                opts.onClickNode($(this).attr('node-id'));
            }
        });*/
      }
    });
    ...
    if (opts.pan) {
      //$chartContainer.css('overflow', 'scroll');
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
      ...
      .append(isNodeActive ? '<div class="org-add title">' + nodeData[opts.nodeTitle] + '</div>' : '<div class="org-add titleInactive">' + nodeData[opts.nodeTitle] + '</div>')
      .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (nodeData[opts.nodeContent] || '') + '</div>' : '')
      .append(isSubInv ? '<div class="footerSub"></div>' : '<div class="footer"><span class ="fa fa-plus-circle"></span>Add Child</div>');
      
    opts.populateData(nodeData);
    
    $nodeDiv.find('.footer').click(function(){
        if(opts.onClickNode !== null){
            opts.onClickNode(nodeData);
        }
    });
    
    $nodeDiv.find('div.content').click(function(){
        if(opts.onClickNodeContent !== null){
            opts.onClickNodeContent(nodeData);
        }
    });
    
    $nodeDiv.find('.org-add, div.content').dblclick(function(){
        if(opts.onDblClickNode !== null){
            opts.onDblClickNode(nodeData);
        }
    });
