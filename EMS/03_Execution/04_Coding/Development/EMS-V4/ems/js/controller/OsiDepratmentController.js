...
OsiDepratmentController.$inject = ['$q', '$scope', '$rootScope', '$window', '$location',
    '$http', '$localStorage', 'OsiDepratmentService', 'FlashService', '$timeout','appConstants','employeeBasicInfoService'];
function OsiDepratmentController($q, $scope, $rootScope, $window, $location, $http,
        $localStorage, OsiDepratmentService, FlashService, $timeout,appConstants,employeeBasicInfoService) {
    var vm = this;
...
$scope.editSelectedRowDetails = function(OsiDepratment){
    ...
    function validateDuplicateDept(OsiDepratment){
    	if($scope.headername=='Edit Department' && $scope.editDeptShortName.toUpperCase()!=OsiDepratment.deptShortName.toUpperCase()){
    		for(var i=0;i<$scope.practiceList.length;i++){
        		if(OsiDepratment.deptShortName.toUpperCase()==$scope.practiceList[i].deptShortName.toUpperCase()){
    				$rootScope.isTrascError = true;
    				FlashService.Error('Department Short Name already exists.');
    				$scope.continuesave = false;
    				break;
    			}
    		}
    	}
    	if($scope.headername!='Edit Department'){
    		for(var i=0;i<$scope.practiceList.length;i++){
        		if(OsiDepratment.deptShortName.toUpperCase()==$scope.practiceList[i].deptShortName.toUpperCase()){
    				$rootScope.isTrascError = true;
    				FlashService.Error('Department Short Name already exists.');
    ...
    $scope.saveOsiDepratment = function (OsiDepratment) {
        ...
        if(!$scope.OsiDepratment.deptShortName){
            $rootScope.isTrascError = true;
            FlashService.Error('Please Enter Department Short Name');
            $scope.continuesave = false;
        } else if (!$scope.OsiDepratment.deptLongName){
            $rootScope.isTrascError = true;
            FlashService.Error('Please Enter Department Long Name');
            $scope.continuesave = false;
        } else {
                                ...
                                function (result) {
                                    ...
                                    if(saveGrades) {
                                        angular.forEach(vm.costingInfo, function(value, key) {
                                            if(value.deptId == undefined || value.deptId == null || value.deptId !== result.deptId) {
                ...
                OsiDepratmentService
                        ...
                        .then(
                                function (result) {
                                    ...
                                    if(saveGrades) {
                                        angular.forEach(vm.costingInfo, function(value, key) {
                                            if(value.deptId == undefined || value.deptId == null) {
                                                value.deptId = result.deptId;
                                            }
                                        });
                                        console.log(vm.costingInfo);
...
// Practice Grades History
    ...
    $scope.getccGradesHistory = function(costing){
        ...
        if(costing.deptId != undefined && costing.orgId != undefined && costing.gradeId != undefined) {
            OsiDepratmentService.getDeptGradesHistory(costing.deptId, costing.orgId, costing.gradeId).then(function (data) {
                ...
                $scope.locModelHeading = "Department Costing History";
