...
function EmployeesController(configData, $q, $scope, $rootScope, $window, $location, $http, $cookieStore,
					...
					organizationService.getOrgById(result.orgId).then(function(data){
						...
						$localStorage.orgName = data.orgName;
						 ...
						 if($localStorage.isUserResponsibility){
							 		...
							 		$(".Employee_List").removeClass("dynatree-active");
						     } else if($localStorage.isEmpOffice) {
								window.location.href = "#/officeInfo-it";
							 }
    ...
    $scope.init = function () {
		...
		$scope.navigateToResponsibility = false;
		...
		if(urlPath.includes('employees-resp') || urlPath.includes('employees-resp-global')) {
			$scope.navigateToResponsibility = true;
			$localStorage.isUserResponsibility = true;
			$localStorage.isEmpOffice = false;
		} else if(urlPath.includes('employees-office')) {
			$scope.navigateToResponsibility = true;
			$localStorage.isUserResponsibility = false;
			$localStorage.isEmpOffice = true;
		}
		$localStorage.globalSearch = false;
		if($location.path().includes('pmosearch')) {
			$localStorage.globalSearch = true;
		}
    };
    ...
    vm.searchEmployee = function (searchempDate,fromOsiEmployeesBtn) {
		...
		$localStorage.globalSearch = false;
		if($location.path().includes('pmosearch') || $location.path().includes('employees-resp-global')) {
			$localStorage.globalSearch = true;
			$scope.searchData = {employeeId: $rootScope.globals.userDTO.id+"", employeeNumber:$scope.employeeNumber,firstName:$scope.firstName,lastName:$scope.lastName,officeEmail:$scope.officeEmail,searchDate:searchempDate};
		} else 
	    	$scope.searchData = {employeeId: $rootScope.globals.userDTO.id+"", employeeNumber:$scope.employeeNumber,firstName:$scope.firstName,lastName:$scope.lastName,officeEmail:$scope.officeEmail,searchDate:searchempDate, orgId: $localStorage.orgId+"", orgCode: $rootScope.orgCode};
    ...
    $scope.searchOsiEmployeess = function () {
		...
		$scope.searchData = {employeeId: $rootScope.globals.userDTO.id+"", employeeNumber:$scope.employeeNumber,firstName:$scope.firstName,lastName:$scope.lastName,officeEmail:$scope.officeEmail};
		var searchData = JSON.stringify($scope.searchData);
...
$scope.setSearchDate= function(searchDate){
	$scope.searchempDate = searchDate;
}
