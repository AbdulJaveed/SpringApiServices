...
$scope.editSelectedRowDetails = function(OsiTitles){
   ...
   /* 	OsiTitlesService.getAllorganizations().then(function(data){
	...
	$scope.operationsGenericFunction = function (doFunction, url) {
	 ...
	 function viewOsiTitlesDetails(OsiTitles) {
	        $scope.headername = "View Job Code";
	        $rootScope.isTrascError = false;
	 ...
	 function editOsiTitlesDetails(OsiTitles) {
        $scope.headername = "Edit Job Code";
        $rootScope.isTrascError = false;
        OsiTitlesService.getOsiTitles(OsiTitles.titleId).then(function (data) {
            $scope.OsiTitles = data;
    ...
    function toggleButtons() {
	 ...
	 function createOsiTitles() {
        $scope.headername = "Create Job Code";
        $rootScope.isTrascError = false;
        $scope.iseditable = true; 
        $scope.iscodeeditable =true;
        $scope.isUpdatable=false;
    ...
    function validateDuplicateTitleEntry(OsiTitles){
    	...
    	if($scope.headername=='Edit Job Code' && $scope.editTitleName!=null && $scope.editTitleName.toUpperCase()== OsiTitles.titleShortName.toUpperCase() && $scope.editTitleCode.toUpperCase()== OsiTitles.titleCode.toUpperCase()){
    		//Do Nothing
    	}
    	if($scope.headername=='Edit Job Code' && $scope.editTitleName!=null  && $scope.editTitleName.toUpperCase()!= OsiTitles.titleShortName.toUpperCase() ){
    		for(var i=0;i<$scope.titleList.length;i++){
        		if( OsiTitles.titleShortName.toUpperCase()==$scope.titleList[i].titleShortName.toUpperCase()){
        			 ...
        			 FlashService.Error('Job Code Short Name already exists.'); 
        			 $scope.continuesave = false;
    				 break;
    			}
    		}
    	}
    	if($scope.headername=='Edit Job Code' && $scope.editTitleName!=null && $scope.editTitleCode.toUpperCase()!= OsiTitles.titleCode.toUpperCase() ){
    		for(var i=0;i<$scope.titleList.length;i++){
        		if( OsiTitles.titleCode.toUpperCase()==$scope.titleList[i].titleCode.toUpperCase()){
    				$rootScope.isTrascError = true;
    				FlashService.Error('Job Code Code already exists.'); 
    				$scope.continuesave = false;
    				break;
    			}
    		}
  ...
  /*  	
    	...
    	if(!($scope.headername=='Edit Job Code')){
    		for(var i=0;i<$scope.titleList.length;i++){
    			if( OsiTitles.titleShortName.toUpperCase()==$scope.titleList[i].titleShortName.toUpperCase()){
       			 $rootScope.isTrascError = true;
       			 FlashService.Error('Job Code Short Name already exists.'); 
 ...
 $rootScope.isTrascError = true;
 FlashService.Error('Please Enter Job Code Short Name'); 
 $scope.continuesave = false;}else if(!$scope.OsiTitles.titleLongName){ 
 $rootScope.isTrascError = true;
 FlashService.Error('Please Enter Job Code Long Name'); 
 $scope.continuesave = false;}else if(!$scope.OsiTitles.titleCode){ 
 $rootScope.isTrascError = true;
 FlashService.Error('Please Enter Job Code'); 
 $scope.continuesave = false;}
