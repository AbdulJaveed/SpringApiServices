...
//                         }
    ...
    function initController() {
    	if(vm.employeeId !==  undefined && vm.employeeId !== null && vm.employeeId!=$rootScope.globals.userDTO.id){
    		$localStorage.isUserResponsibility = false;
    ...
    function getOsiUserResponsibilities(){
    	if(vm.employeeId !== undefined && vm.employeeId !== null){
	    	ResponsibilityService.getResponsibilities(vm.employeeId).then(function(data){
	    		if(data.length > 0)
					vm.userResp = data;
					angular.forEach(vm.userResp, function(resp, key){
						if(resp.startDate)
						resp.startDate = $filter('date')(new Date(resp.startDate), 'yyyy-MM-dd');
						if(resp.endDate == configData.maxEffecEndDate)
							resp.endDate = '';
						else if(resp.endDate){
							resp.endDate = $filter('date')(new Date(resp.endDate), 'yyyy-MM-dd');
						}
					});
    ...
    $scope.next = function() {
        ...
        angular.forEach(vm.userResp, function(value, key){
			...
			if(value.endDate == '' || value.endDate == undefined){
				value.endDate = configData.maxEffecEndDate;
			}
        })
        if(emptyError){
        	$rootScope.isTrascError = true;
			...
			if ($scope.wrongSelectedResp) {
				...
				angular.forEach(vm.userResp, function(value, key) {
					...
					userRespObj.startDate = value.startDate+" 00:00:00";
					if(value.endDate && value.endDate != configData.maxEffecEndDate)
					userRespObj.endDate = value.endDate+" 23:58:58";
				else{
				...
				ResponsibilityService.setResponsibilities(userRespList).then(function(data){
					...
					angular.forEach(userRespList, function(value, key) {
						
						userRespList[key].startDate = $filter('date')(new Date(value.startDate), 'yyyy-MM-dd');
						if(userRespList[key].endDate && userRespList[key].endDate != configData.maxEffecEndDate)	
							userRespList[key].endDate = $filter('date')(new Date(value.endDate), 'yyyy-MM-dd');
						else{
							userRespList[key].endDate = "";
						}					
					});
					vm.userResp = userRespList;
				}).catch(function(data){
    ...
    vm.compareDate = function(userResp){
		if(userResp.endDate == undefined || userResp.endDate == configData.maxEffecEndDate){
			userResp.endDate = '';
		} 
