...
$scope.editSelectedRowDetails = function(OsiGrades){
     ...
     $scope.isSelectedOrg = OsiGrades.orgId;
     toggleButtons();
}
    $timeout(function () {
...
$scope.closeGradeHistModal = function() {
    $('#GradeHistModal').modal('hide');
}
...
$scope.getGradesHistory = function(){
    if($scope.isSelectedRow != undefined && $scope.isSelectedOrg != undefined) {
        OsiGradesService.getGradesHistory($scope.isSelectedRow, $scope.isSelectedOrg).then(function (data) {
            $scope.gradeHistList = data;
            angular.forEach($scope.gradeHistList, function(grade, k){
                angular.forEach($scope.organizations, function(org, key) {
                    if(org.orgId == grade.orgId) {
                        $scope.gradeHistList[k].orgName = org.orgName;
                    }
                });
            }) 
            $scope.locModelHeading = "Grades History";
           
            $('#OsiGrades-model').modal({backdrop: 'static', keyboard: false});
            $('#GradeHistModal').modal('toggle'); 
        }).catch(function(error){
            var msg=appConstants.exceptionMessage;
            if(error.data.httpStatus){ 
                msg=error.data.errorMessage; 
