        ...
        for (var i = 0; i < vm.operations.length; i++) {
            ...
            for (var j = 0; j < items.length; j++) {
                ...
                if (vm.operations[i].id == items[j].osiOperations.id && items[j].opUrl != "") {
                    //urlObj.startDate = $filter('date')(new Date(items[j].startDate), 'dd-MM-yyyy');
                    //urlObj.endDate = $filter('date')(new Date(items[j].endDate), 'dd-MM-yyyy');
                    urlObj.startDate = items[j].startDate;
                    urlObj.endDate = items[j].endDate;
                            ...
                            angular.forEach(vm.functionOperations, function(value,
                            	...
                            	var osiOperations;
										 ...
										 if(value.startDate != undefined || value.startDate != undefined !=""){
											//value.startDate = $filter('date')(new Date(value.startDate), 'dd-MM-yyyy');
                                            //value.endDate = $filter('date')(new Date(value.endDate), 'dd-MM-yyyy');
										 }
