(function () {
    ...
    function OsiDepratmentService($http, $cookieStore, $rootScope, $timeout, $localStorage, $sessionStorage,configData,$q) {
        ...
        service.getDeptGradesHistory = getDeptGradesHistory;
        ...
        function getDeptGradesHistory(deptId, orgId, gradeId){
            return $http.get(configData.url+'crud/'+domain+'/getOsiDeptGradesHistory/'+deptId+'/'+orgId+'/'+gradeId).then(handleSuccess, handleError('Error getting grades history'));
        }
