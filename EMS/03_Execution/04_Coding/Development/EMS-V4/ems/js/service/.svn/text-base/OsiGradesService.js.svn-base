(function () {
    ...
    function OsiGradesService($http, $cookieStore, $rootScope, $timeout, $localStorage, $sessionStorage,configData,$q) {
        ...
        service.getGradesHistory = getGradesHistory;
        service.getAllOsiTitleGrades = getAllOsiTitleGrades;
        ...
        function getGradesHistory(gradeId, orgId){
            return $http.get(configData.url+'crud/'+domain+'/getOsiGradesHistory/'+gradeId+'/'+orgId).then(handleSuccess, handleError('Error getting grades history'));
        }
        
        function getAllOsiTitleGrades(){
            return $http.get(configData.url+'list/'+domain+'/getAllOsiTitleGrades',config).then(handleSuccess, handleError('Error getting all orgnaizations'));
        }
